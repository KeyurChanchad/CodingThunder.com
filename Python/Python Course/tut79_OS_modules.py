...
os.chdir( ):        it is used in case we want to change our directory. The new path is sent inside the parenthesis. If we want to access some files or folder from some other directory, we can use it.
...
os.path.join( ):         It joins one or more path components.
                        We can join the paths by simply using a + sign, but the benefit of using this function is that
                        we do not have to worry about extra slashes between the components.
...
os.path.isdir( ):        It returns true if the path is an existingdirectory.
"""
import os
print(dir(os))
print(os.getcwd())
