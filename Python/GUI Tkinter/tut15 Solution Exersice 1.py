...
root = Tk()
root.title("CodeWithKeyur News - Aapka Apna Akhabaar")
...
for i in range(0, 3):
    ...
    # Image.ANTIALIAS is use for clear photo
    image = image.resize((200,170), Image.ANTIALIAS)
    photos.append(ImageTk.PhotoImage(image))

f0 = Frame(root, width=800, height=70)
Label(f0, text="Code With Keyrus's News", font="lucida 33 bold").pack()
Label(f0, text="January 19, 2050", font="lucida 13 bold").pack()
f0.pack()

'''
f1 = Frame(root, width=900, height=200, pady=14)
Label(f1, text=texts[0], padx=20, pady=22).pack(side="left")
...
'''
for i in range(3):
    f = Frame(root, width=900, height=200, pady=20)
    Label(f, text=texts[i], padx=12, pady=12).pack(side="left")
